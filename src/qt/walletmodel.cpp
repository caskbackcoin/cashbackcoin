...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered cashback address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CASH.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal cashback:URI (cashback:XyZ...?message=example)
            {
...
static void NotifyzCASHReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCASHReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCASHReset.connect(boost::bind(NotifyzCASHReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCASHReset.disconnect(boost::bind(NotifyzCASHReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcashTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
