...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCaskbackAmount"))
        settings.setValue("nAnonymizeCaskbackAmount", 1000);

    nAnonymizeCaskbackAmount = settings.value("nAnonymizeCaskbackAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCaskbackAmount"))
        SoftSetArg("-anonymizecashbackamount", settings.value("nAnonymizeCaskbackAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cashback.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCaskbackAmount:
            return QVariant(nAnonymizeCaskbackAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCaskbackAmount:
            nAnonymizeCaskbackAmount = value.toInt();
            settings.setValue("nAnonymizeCaskbackAmount", nAnonymizeCaskbackAmount);
            emit anonymizeCaskbackAmountChanged(nAnonymizeCaskbackAmount);
            break;
        case CoinControlFeatures:
