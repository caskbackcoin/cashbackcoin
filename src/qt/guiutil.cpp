...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Caskback address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Caskback: URI
...
{
    // Convert cashback:// to cashback:
    //
    //    Cannot handle this later, because cashback:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open cashback.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Caskback.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Caskback.lnk
...
{
    return GetAutostartDir() / "cashback.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a cashback.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Caskback\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the cashback app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add cashback app to startup item list
