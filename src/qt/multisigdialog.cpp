...
bool MultisigDialog::createMultisigTransaction(vector<CTxIn> vUserIn, vector<CTxOut> vUserOut, string& feeStringRet, string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw runtime_error("Not enough CASH provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
            ...
            throw runtime_error("Not enough CASH provided to cover fee");
        }

        //clear ja from script sigs
        for(CTxIn& in : tx.vin){
            in.scriptSig.clear();
        }
        multisigTx = tx;
