...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    if (nValue < 0 && !AllowNegative)
        return "<span>" + _("anown") + "</span>";

    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::CASH, nValue);
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        std::string OutputsContentCells[] =
            {
                itostr(i),
                (HashNext == uint256S("0")) ? (fAddrIndex ? _("no") : _("anown")) : "<span>" + makeHRef(HashNext.GetHex()) + ":" + itostr(nNext) + "</span>",
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (cashback.conf).");
            QMessageBox::warning(this, "Caskback Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
