...
#ifndef Caskback_ZCASHWALLET_H
#define Caskback_ZCASHWALLET_H
...
public:
    CzCASHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCASH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCASH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Caskback_ZCASHWALLET_H
