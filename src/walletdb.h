...
public:
    ...
    int nVersion;
    int64_t nCreateTime; // 0 means anown
...
public:
    ...
    bool WriteZCASHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCASHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCASHSeed_deprecated(uint256& seed);
    bool EraseZCASHSeed();
    bool EraseZCASHSeed_deprecated();

    bool WriteZCASHCount(const uint32_t& nCount);
    bool ReadZCASHCount(uint32_t& nCount);
