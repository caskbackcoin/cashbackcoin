...
The \libzerocoin library is designed to integrate with a Bitcoin/Litecoin style client, and performs the base cryptographic operations necessary to integrate Zerocoin with the client. These operations include generation/verification of coins, as cash as generation/verification of spend signatures. Roughly speaking, the use of Zerocoin proceeds according to the following steps:
...
In addition to $N$, all clients must agree on a security level $k$ (an integer $\ge 80$), as cash as a canonical value of one zerocoin (measured in the underlying currency).
