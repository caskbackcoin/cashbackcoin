...
bool static Socks5(string strDest, int port, const ProxyCredentials *auth, SOCKET& hSocket)
{
    ...
    if (pchRet2[1] != 0x00) {
        ...
        default:
            return error("Proxy error: anown");
        }
    }
    if (pchRet2[2] != 0x00) {
        CloseSocket(hSocket);
...
std::vector<unsigned char> CNetAddr::GetGroup() const
{
    ...
    // includes mapped IPv4, SIIT translated IPv4, and the cash-known prefix
...
int CNetAddr::GetReachabilityFrom(const CNetAddr* paddrPartner) const
{
    ...
    case NET_TOR:
        ...
        default:
            return REACH_DEFAULT;
        case NET_IPV4:
            return REACH_IPV4; // Tor users can connect to IPv4 as cash
        case NET_TOR:
