...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cashback.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cashbackd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CASHand zCASHmoney supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 17773, 27293));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CASH/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CASH/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, cashback, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CASH/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Caskback specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-cashstake=<n>", strprintf(_("Enable or disable staking functionality for CASHinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcashstake=<n>", strprintf(_("Enable or disable staking functionality for zCASHinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:17773"));
    ...
    strUsage += HelpMessageOpt("-backupzcash=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCASHminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcashbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCASHbackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as cash, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizecashbackamount=<n>     " + strprintf(_("Keep N CASHanonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The Caskback Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Caskback is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Caskback is shutting down."));
...
#endif
    // Make sure only a single Caskback process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Caskback is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Caskback version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (!bitdb.Open(GetDataDir())) {
            ...
            try {
                ...
                // failure is ok (cash, not really, but it's not worse than what we started with)
            }
    ...
    // -noproxy (or -proxy=0) as cash as the empty string can be used to not set a proxy, this is the default
    ...
    //Caskback: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Caskback specific: zerocoin and spork DB's
                ...
                // Caskback: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCASHMinted();
                        RecalculateZCASHSpent();
                    }
                    RecalculateCASHSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Caskback: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Caskback") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Caskback to complete") << "\n";
        ...
        zwalletMain = new CzCASHWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCASHWallet
        uiInterface.InitMessage(_("Syncing zCASHwallet..."));

        bool fEnableZZcrBackups = GetBoolArg("-backupzcash", true);
        pwalletMain->setZZcrAutoBackups(fEnableZZcrBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zcashTracker->Init();
    ...
    if (readResult == CMasternodeDB::FileError)
        ...
        if (readResult == CMasternodeDB::IncorrectFormat)
            ...
            LogPrintf("file format is anown or invalid, please fix it manually\n");
    }
...
//    }
//
//    nAnonymizeCaskbackAmount = GetArg("-anonymizecashbackamount", 0);
//    if (nAnonymizeCaskbackAmount > 999999) nAnonymizeCaskbackAmount = 999999;
//    if (nAnonymizeCaskbackAmount < 2) nAnonymizeCaskbackAmount = 2;
    ...
    LogPrintf("Anonymize Caskback Amount %d\n", nAnonymizeCaskbackAmount);
