...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 CASHat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCASHsend
enum ZerocoinSpendStatus {
    ZCASH_SPEND_OKAY = 0,                            // No error
    ZCASH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCASH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCASH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCASH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCASH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCASH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCASH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCASH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCASH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCASH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCASH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCASH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCASH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCASH_SPENT_USED_ZCASH= 14,                      // Coin has already been spend
    ZCASH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCASH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZCASHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzcashAuto) const;
    ...
    */
    ...
    CzCASHWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCASHTracker> zcashTracker;
    ...
    void setZWallet(CzCASHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zcashTracker = std::unique_ptr<CzCASHTracker>(new CzCASHTracker(strWalletFile));
    }

    CzCASHWallet* getZWallet() { return zwalletMain; }
    ...
    /** zCASHreset */
    boost::signals2::signal<void()> NotifyzCASHReset;
