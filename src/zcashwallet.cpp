...
CzCASHWallet::CzCASHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcash seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCASHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCASHSeed_deprecated()) {
                    LogPrintf("%s: Updated zCASHseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcash seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcash wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCASHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCASHCount(nCountLastUsed);
    else if (!walletdb.ReadZCASHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCASHWallet::Lock()
{
    seedMaster = 0;
}

void CzCASHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCASHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCASH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCASHWallet::LoadMintPoolFromDB()
{
...
void CzCASHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCASHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCASHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcashTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCASHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCASH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcashTracker which also adds to database
    pwalletMain->zcashTracker->Add(dMint, true);
...
void CzCASHWallet::SeedToZCASH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCASHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCASHWallet::GenerateDeterministicZCASH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCASHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCASHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
