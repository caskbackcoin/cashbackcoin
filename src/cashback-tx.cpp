...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Caskback cashback-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  cashback-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded cashback transaction") + "\n" +
                               "  cashback-tx [options] -create [commands]   " + _("Create hex-encoded cashback transaction") + "\n" +
                               "\n";
...
static void RegisterLoad(const string& strInput)
{
    ...
    // load file chas into one big buffer
    string valStr;
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    if (flagStr.size() > 0)
        if (!findSighashFlags(nHashType, flagStr))
            throw runtime_error("anown sighash flag/sign option");
...
static void MutateTx(CMutableTransaction& tx, const string& command, const string& commandVal)
{
    ...
    if (command == "nversion")
        ...
        throw runtime_error("anown command");
}
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded cashback transaction
