...
int nAnonymizeCaskbackAmount = 1000;
 ...
 * by cash-meaning people adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cashback" is a composite category enabling all Caskback-related debug output
            if (ptrCategory->count(string("cashback"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cashback.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cashback.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cashback.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cashbackd.pid"));
